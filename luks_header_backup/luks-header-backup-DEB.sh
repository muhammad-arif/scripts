...
# Declaring Variables
DIR="/tmp/luks"
LDIR="/dev/mapper/*crypt"
FILE1="luks_essential.info"
FILE2="luks_header_dump.bin"
FILE3="luks_keys.bin"
FSTAB="/etc/fstab"
BCKFL="$(hostname)_luks_backup.gz"
TMP="/tmp/"
LKS="luks/"
...
for i in $(ls $LDIR); do dmsetup table --target crypt --showkeys $i | sed -E -n 's/.*crypt\s[a-zA-Z0-9-]+\s([a-f0-9]+)\s.*$/\1/p' | sed 's/\([0-9A-F]\{2\}\)/\\\\\\x\1/gI' | xargs printf > $DIR/$(echo $i|awk -F "/" '{print $4}')-key.bin; done && success || die "No Device found. || STEP -4 ||"
